...
def print_results(results_dic, results_stats_dic, model, 
    ...
    if (print_incorrect_dogs and ( (results_stats_dic['num_correctDogs'] + results_stats_dic['num_correctNotDogs'])!= results_stats_dic['num_images'] )):
        print("\nINCORRECT Dog/NOT Dog Assignments:")
        for key in results_dic:
            if sum(results_dic[key][3:]) == 1:
                print("Real: %-26s   Classifier: %-30s" %(results_dic[key][0],results_dic[key][1]))
    if (print_incorrect_breed and(results_stats_dic['num_correctDogs'] !=results_stats_dic['num_correctBreed']) ):
